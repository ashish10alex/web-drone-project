...
PairedComparisonPageManager.prototype.createPages = function (_pageManager, _pageTemplateRenderer, _pageConfig, _audioContext, _bufferSize, _audioFileLoader, _session, _errorHandler, _language) {
  ...
  // shuffle(this.conditions); // prevent shuffle to make backend storage easier
  
  for (var i = 0; i < this.conditions.length-1; ++i) {
  	var page = new PairedComparisonPage(this.reference, this.conditions[i], this.conditions[i+1], _pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler, _language);
